...
clc;
clear;
gray = double(imread('Butterfly.png'));               %读入初始图,作为初始迭代分布振幅 
...
figure(1)
subplot(131);imshow(mat2gray(gray));title('原图');
subplot(132);imshow(mat2gray(abs(G0_FieNew)));title('相位原件分布(Fie)');
subplot(133);imshow(mat2gray(abs(fftshift(g0_FieNew))));title('模拟衍射输出(Fie)');
